...
for i in range(6):
    asteroid = Enemy("asteroid.png", randint(0, w-50), -40, 50, 50, randint(1,5))
...
restart = GameSprite("restart.png", 240, 200, 111, 62, 0)
...
while game:
    for e in event.get():
        if e.type == QUIT:
            game = False
        #if e.rype == MOUSEBUTTONDOWN and e.button == 1:
        if e.type == KEYDOWN:
            if e.key == K_SPACE:
    ...
    if not finish:
        ...
        if sprite.spritecollide(ship, asteroids, False):
            restart.draw()
            lose_text = mainfont.render("YOU LOSE", True, (255,255,255))
            window.blit(lose_text, (220, 220))
            finish = True
